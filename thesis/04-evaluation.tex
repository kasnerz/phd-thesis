...
\chapter{Evaluating Text Faithfulness}
\label{chap:evaluation}


In this chapter, we introduce two approaches for evaluating \emph{semantic accuracy} of \ac{d2t} generation. As discussed in \autoref{sec:evaluation}, it is essential that the texts based on data contain information faithful to the input data. Yet, there are no widely applicable automatic metrics to assess the semantic accuracy of generated texts. Our metrics are based on \acp{plm} and generic text-to-text operations, which makes our approaches applicable to various tasks and domains.

In \autoref{sec:sem-acc}, we first describe a metric which is suitable for detecting omissions and hallucinations in \ac{d2t} generation from \emph{knowledge graphs}. The metric is based on an off-the-shelf \ac{plm} for \ac{nli}, which we repurpose for the task of assessing semantic accuracy. We show that our metric correlates well with human judgements, and that in some cases the metric even provides judgements which are more accurate.

In \autoref{sec:tok-eval}, we focus on detecting factual errors in \ac{d2t} generation from \emph{complex tabular data}. We propose a metric based on a neural \ac{plm}-based tagger, which can mark individual tokens with fine-grained error categories. To provide relevant information for the tagger, we combine a rule-based generation and neural-based retrieval model. With our metric, we placed first out of four automatic metrics in the Shared Task on Evaluating Accuracy in Generated Texts.
...
\paragraph{Data Preprocessing} Throughout \autoref{chap:low-res}, we have used simple templates for transforming individual triple to facts, i.e., simple sentences capturing the meaning of the triple.  We use the same method here, considering two cases:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item  \textsc{Rough}: The three failure modes are combined into \emph{not\_OK}. The 2-way output corresponds more to a usage inside an \ac{d2t} generation system for output reranking or filtering: any output that is \emph{not\_OK} should be penalized or filtered out.
\end{itemize}
...
\paragraph{WebNLG Analysis} The overall scores show that our metric deviates quite a lot from the human judgments. Our manual error analysis indicates several reasons:
\begin{enumerate}
...
\paragraph{Context Retrieval} The maximum length of the input sequence for our error tagger is 512 tokens, which is about 10\% of the total length of the generated sentences $G$. Therefore, we select only a subset of $G$, which we refer to as \emph{context} $C$. For each generated sentence $g_i\in G$, we measure semantic similarity between $g_i$ and the evaluated sentence $s$ using Sentence Transformers \cite{reimers-gurevych-2019-sentence}.\footnote{\url{https://www.sbert.net/}}
In particular, we embed the sentence tokens by applying mean pooling on the output of \texttt{paraphrase-distilroberta-base-v2}, getting the embedding vectors $e_{s}$ and $e_{g_i}$. Then we compute the cosine similarity between the embeddings:
\begin{align}
...
\paragraph{Synthetic Data} The gold-standard data contains only 60 games, as opposed to 3,395 games in the Rotowire training set. This led us to an idea of using the training set as a source of synthetic data, introducing errors into human-written descriptions. We focus only on the \errent{NAME} and \errnum{NUMBER} errors, i.e., the categories which are the most represented and also easiest to generate. In each sentence, we identify named entities in the text using \emph{spaCy}.\footnote{\url{https://spacy.io}} We modify only certain portion of entities according to the \emph{entity modification rate} (EMR), which we treat as a hyperparameter. We introduce the \errent{NAME} errors by:
\begin{enumerate}
