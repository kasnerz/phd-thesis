...
\paragraph{Self-attention Mechanism} Self-attention \cite{cheng2016long} is a variant of the attention mechanism in which the source and the target sequence is identical. Here, we describe the variant introduced by \citet{vaswani2017attention}. For the input $\mathbf{X} \in \mathbb{R}^{n,d}$, the \emph{self-attention} produces the output $\mathbf{H} \in \mathbb{R}^{n,d}$ of the same size. For each token, the resulting vector $\mathbf{h}_i \in \mathbf{H}$ is a weighed combination of the value vectors corresponding to all the tokens in a sequence (including the token itself):
\begin{align}
...
\paragraph{Transformer Architecture} The \emph{transformer}\footnote{Although \citet{vaswani2017attention} uses ``Transformer'' with a capital ``T'', the orthography is gradually shifting towards the variant with a lower-case ``t''. See, e.g., \citet[p.~215]{jurafsky2024}.} \cite{vaswani2017attention} is a neural network architecture which gets rid of another limitation of \acp{rnn} -- the sequential processing. The parallelizability is enabled by replacing the hidden state (which previously served for sharing information among tokens in a single sequence) with the self-attention mechanism.


The self-attention mechanism is a part of the so-called transformer block, which is repeated over. The block also contains an \ac{mlp}, residual connections, and normalizing layers.

As shown in \autoref{fig:transformer}, the original Transformer architecture follows the encoder-decoder framework. The encoder consists of $N_e$ layers, each layer consisting of a self-attention layer and an \ac{mlp}.
