...
\paragraph{Text Fluency} Text fluency is a catch-all term for measuring grammatical correctness, spelling, word and stylistical choices of text \cite{celikyilmazEvaluationTextGeneration2021}. Since texts more similar to the human written text tend to be more fluent, lexical and semantic similarity metrics are often used as a proxy for measuring text fluency. Alternatively, we can use the \emph{perplexity} of the text under a neural \ac{lm}, assuming that the \ac{lm} assigns lower probability to less probable sentences \cite{leeFactualityEnhancedLanguage2022,kaneNUBIANeUralBased2020}.


\paragraph{Lexical Diversity} Lexical diversity measures the variability and richness of expressions in the text \cite{vanmiltenburgMeasuringDiversityAutomatic2018}. A basic measure is the number of unique $n$-grams normalized by text length. \Ac{ttr} measures the ratio between the average number of different words and the total number of words   \cite{johnson1944studies}.
% We can also measure the number of distinct unigrams and bigrams \cite{li2016diversity}. 
Another view is using measuring entropy of the text.
Lexical diversity is not generally a required aspect of output texts in \ac{d2t} generation, although there are some approaches which explicitly aim to decode diverse outputs \cite{hanGeneratingDiverseDescriptions2021,perlitzDiversityEnhancedTabletoText2022}.
