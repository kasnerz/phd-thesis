...
\begin{figure}[t]
    ...
    \caption{A scheme of the common objectives used by pretrained models: 1)  language modeling (decoder-only), 2) masked language modeling (encoder-only), 3) denoising (encoder-decoder). The special symbol $\bullet$ (beginning of a sentence) is used to bootstrap the decoding process.}\label{fig:objectives}
...
In this section, we describe the \ac{d2t} generation datasets, with a strong focus on the datasets used later on in the thesis. We do not describe our custom datasets presented in \cite{kasnerMindLabelsDescribing2022} and \cite{kasnerReferenceBasedMetricsAnalyzing2024}; these are described in their respective sections in \autoref{chap:investigating}.

\begin{table*}[t]
    \centering\footnotesize
    \begin{tabular}{@{}llrrr@{}}
        \toprule
        \multirow{2}{*}{\textbf{Dataset}}                                                                  & \multirow{2}{*}{\textbf{Data Type}} & \multicolumn{3}{c}{\textbf{Number of examples}}                                \\  \cmidrule(lr){3-5}
                                                                                                           &                                     & \textbf{train}                                  & \textbf{dev} & \textbf{test} \\\midrule
        CACAPO \cite{vanderleeCACAPODatasetMultilingual2020}                                               & Key-value                           & 15,290                                          & 1,831        & 3,028         \\
        DART \cite{nan2021dart}                                                                            & Graph                               & 62,659                                          & 2,768        & 5,097         \\
        \textbf{E2E} \cite{dusekSemanticNoiseMatters2019,dusekEvaluatingStateoftheartEndtoEnd2020}         & Key-value                           & 33,525                                          & 1,484        & 1,847         \\
        EventNarrative \cite{colas2021eventnarrative}                                                      & Graph                               & 179,544                                         & 22,442       & 22,442        \\
        HiTab \cite{chengHiTabHierarchicalTable2021}                                                       & Table w/hl                          & 7,417                                           & 1,671        & 1,584         \\
        Chart-To-Text \cite{kantharajCharttoTextLargeScaleBenchmark2022}                                   & Chart                               & 24,368                                          & 5,221        & 5,222         \\
        Logic2Text \cite{chenLogic2TextHighFidelityNatural2020}                                            & Table w/hl                          & 8,566                                           & 1,095        & 1,092         \\
        LogicNLG \cite{chenLogicalNaturalLanguage2020}                                                     & Table                               & 28,450                                          & 4,260        & 4,305         \\
        NumericNLG \cite{suadaaTabletoTextGenerationNumerical2021}                                         & Table                               & 1,084                                           & 136          & 135           \\
        SciGen \cite{moosaviLearningReasonText2021}                                                        & Table                               & 13,607                                          & 3,452        & 492           \\
        \textbf{SportSett:Basketball} \cite{thomson2020sportsett}                                          & Table                               & 3,690                                           & 1,230        & 1,230         \\
        ToTTo \cite{parikhToTToControlledTableToText2020}                                                  & Table w/hl                          & 121,153                                         & 7,700        & 7,700         \\
        \textbf{WebNLG} \cite{gardentWebNLGChallengeGenerating2017,ferreira2020BilingualBiDirectional2020} & Graph                               & 35,425                                          & 1,666        & 1,778         \\
        WikiBio \cite{lebretNeuralTextGeneration2016}                                                      & Key-value                           & 582,659                                         & 72,831       & 72,831        \\
        WikiSQL \cite{zhong2017seq2sql}                                                                    & Table + SQL                         & 56,355                                          & 8,421        & 15,878        \\
        WikiTableText \cite{bao2018table}                                                                  & Key-value                           & 10,000                                          & 1,318        & 2,000         \\
        \bottomrule
    \end{tabular}
    \caption{The list of external \ac{d2t} datasets used in this work. Boldface denotes the datasets which are used extensively for our experiments, the rest of the datasets was used in \autoref{sec:tabgenie}. Glossary of data types: \textit{Key-value}: key-value pairs, \textit{Graph}: subject-predicate-object triples, \textit{Table}: tabular data (\textit{w/hl}: with highlighted cells), \textit{Chart}: chart data, \textit{SQL}: strings with SQL queries.}
    \label{tab:datasets}.
\end{table*}
