...
\noindent The sampling step (line \ref{alg:dec:sample}) can be realized in multiple ways:
\begin{itemize}
...
\begin{table}[ht]
    ...
    \begin{tabular}{lp{2.5cm}ll}
        ...
        \midrule
        \multirow{2}{*}{Encoder} & \multirow{2}{*}{MLM}           & BERT \cite{devlinBERTPretrainingDeep2019}            & 110M (\texttt{base}), 340M (\texttt{large})  \\
                                 &                                & RoBERTa \cite{liuRoBERTaRobustlyOptimized2019}       & 125M (\texttt{base}), 355M (\texttt{large})  \\
        \midrule
        \multirow{2}{*}{Enc-Dec} & \multirow{2}{*}{Denoising}     & BART \cite{lewisBARTDenoisingSequencetoSequence2019} & 406M (\texttt{large})                        \\
                                 &                                & mBART \cite{liuMultilingualDenoisingPretraining2020} & 880M (\texttt{large})                        \\
                                 &                                & T5 \cite{raffelExploringLimitsTransfer2019}          & 220M (\texttt{small}), 770M (\texttt{large}) \\
        \midrule
        Decoder                  & Causal LM                      & GPT-2 \cite{radfordLanguageModelsAre2019}            & 117M (\texttt{small}), 774M (\texttt{large}) \\
        \bottomrule
    \end{tabular}
    \caption{Types of pretrained models}
