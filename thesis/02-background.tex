...
\begin{itemize}
    \item \textbf{BLEU} \cite{papineni2002bleu} is a metric which measures precision, i.e., to which extent does the information in the generated text correspond to the reference text. BLEU has been originally used for evaluating machine translation, but since then it has become commonplace in \ac{nlp}. The metric is computed as a geometric mean of the $n$-gram precisions, with a brevity penalty to penalize shorter outputs.
    \item \textbf{ROUGE} \cite{lin-2004-rouge} is a set of metrics which focus on recall, i.e., to which extent does the generated text preserve the information in the reference text. ROUGE has been originally designed for evaluating automatic summarization, but since then it has been used widely (and as recently found by \citet{gruskyRogueScores2023}, oftentimes incorrectly) across the \ac{nlp} literature. ROUGE includes several variants, such as ROUGE-L, which measures the longest matching word sequence, and ROUGE-{1/2/3/4}, which measures the overlap on the respective n-grams.
    \item \textbf{METEOR} \cite{banerjee-lavie-2005-meteor} is a metric which balances unigram precision and recall by computing their harmonic mean. METEOR also considers stemming and synonym matching to address the problems of text variability. It has been shown to produce better correlations with human judgements than BLEU \cite{agarwal2008meteor}, but is more complex and more expensive to compute.
...
\paragraph{Semantic Accuracy} Semantic accuracy---also called \emph{faithfulness} or \emph{factual consistency} \cite{celikyilmazEvaluationTextGeneration2021}---measures inacurracies in the output text with respect to the input data. The inacurracies can be broadly divided into \emph{omissions} (the model not mentioning facts in the input data) and hallucinations (the model mentioning extra facts that are not supported by the input data). Further, the hallucinations can be \emph{extrinsic}, i.e., the model introduces external information not present in the data, or \emph{intrinsic}, i.e., the model uses the data incorrectly \cite{maynezFaithfulnessFactualityAbstractive2020}. Naturally, omissions apply only in the case we actually want to mention all the facts present in the input data (i.e., no content selection is needed).

There are no widely accepted metrics for measuring semantic accuracy in \ac{d2t} generation. PARENT \cite{dhingraHandlingDivergentReference2019} is a reference-based metric for table-to-text generation, which uses lexical alignment models for computing precision and recall for tabular values. The closest notion is the \emph{slot error rate} from task-oriented dialogue systems, which is typically implemented by exact string match or regular expressions  \cite{wen2015semantically,dusekEvaluatingStateoftheartEndtoEnd2020}. In \Cref{sec:sem-acc,sec:eval-token}, we present two novel \emph{referenceless} metrics for evaluating semantic accuracy of \ac{d2t} generation using \acp{plm}. In \autoref{sec:prompting}, we also show how to evaluate semantic accuracy of texts using a prompted \ac{llm}.

\paragraph{Text Fluency} Although lexical and semantic similarity metrics are often used also as a proxy for measuring text fluency (texts more similar to the human written text tend to be more fluent), we can also estimate the fluency of the text without any reference. A commonly used method is measuring the \emph{perplexity} of the text under a \ac{plm}. Assuming that the \ac{plm} correctly models the probability of sentences in the training corpus (and the corpus mostly consists of fluent sentences), we can say that the lower the perplexity is, the more probable the sente \cite{leeFactualityEnhancedLanguage2022,kaneNUBIANeUralBased2020}


\paragraph{Text Diversity}

\paragraph{Human Evaluation} Human evalution can focus on similar aspects, but is rarely standardized and usually task-specific \cite{belzDisentanglingPropertiesHuman2020}.
