...
In this section, we describe the \ac{d2t} generation datasets, with a strong focus on the datasets used later on in the thesis. We do not describe our custom datasets presented in \citet{kasnerMindLabelsDescribing2022} and \citet{kasnerReferenceBasedMetricsAnalyzing2024}; these are described in their respective sections in \autoref{chap:investigating}.

\begin{table*}[t]
    \centering\small
    \begin{tabular}{@{}lllr@{}}
        \toprule
        \textbf{Dataset}                                                                           & \textbf{Data Format} & \textbf{Domain} & \textbf{\# Ex.} \\  \midrule
        CACAPO \cite{vanderleeCACAPODatasetMultilingual2020}                                       & Key-value            & Multi-domain    & 20149           \\
        DART \cite{nan2021dart}                                                                    & RDF triples          & Wikipedia       & 70524           \\
        \textbf{E2E} \cite{dusekSemanticNoiseMatters2019,dusekEvaluatingStateoftheartEndtoEnd2020} & Key-value            & Restaurants     & 36856           \\
        EventNarrative \cite{colas2021eventnarrative}                                              & RDF triples          & Events          & 224428          \\
        HiTab \cite{chengHiTabHierarchicalTable2021}                                               & Table w/hl           & Statistics      & 10672           \\
        Chart-To-Text \cite{kantharajCharttoTextLargeScaleBenchmark2022}                           & Table                & Statistics      & 34811           \\
        Logic2Text \cite{chenLogic2TextHighFidelityNatural2020}                                    & Table w/hl           & Wikipedia       & 10753           \\
        LogicNLG \cite{chenLogicalNaturalLanguage2020}                                             & Table                & Wikipedia       & 37015           \\
        NumericNLG \cite{suadaaTabletoTextGenerationNumerical2021}                                 & Table                & Science         & 1355            \\
        SciGen \cite{moosaviLearningReasonText2021}                                                & Table                & Science         & 17551           \\
        \textbf{Rotowire} \cite{wiseman2017challenges}                                             & Table                & Basketball      & 6150            \\
        ToTTo \cite{parikhToTToControlledTableToText2020}                                          & Table w/hl           & Wikipedia       & 136553          \\
        \textbf{WebNLG} \cite{gardentWebNLGChallengeGenerating2017}                                & RDF triples          & DBPedia         & 38869           \\
        WikiBio \cite{lebretNeuralTextGeneration2016}                                              & Key-value            & Biographies     & 728321          \\
        WikiSQL \cite{zhong2017seq2sql}                                                            & Table + SQL          & Wikipedia       & 80654           \\
        WikiTableText \cite{bao2018table}                                                          & Key-value            & Wikipedia       & 13318           \\
        \bottomrule
    \end{tabular}
    \caption{The list of external \ac{d2t} datasets used in this work. The datasets are included in the \textsc{TabGenie} framework (\autoref{sec:tabgenie}), the datasets in \textbf{boldface} are also used in other experiments. Glossary of data types: \textit{Key-value}: key-value pairs, \textit{Graph}: subject-predicate-object triples, \textit{Table}: tabular data (\textit{w/hl}: with highlighted cells), \textit{Chart}: chart data, \textit{SQL}: strings with SQL queries.}
    \label{tab:datasets}.
\end{table*}

\paragraph{Data Formats} Structured data come in a variety of formats. The most common formats are:
...
\begin{itemize}
    ...
    \item \textbf{Table}: Each input is structured as a \textit{table}. In its basic version, a table is a two-dimensional matrix with $m$ columns and $n$ rows, which together define a grid of $m \times n$ cells. The cell can be either a heading (for the respective row or column), or contain a textual or numerical value. Generally, a table can contain merged cells, nested headings, or multiple disjoint table regions.
\end{itemize}
...
\paragraph{Selected Datasets}

\begin{itemize}
    \item \textbf{WebNLG}: The WebNLG dataset \cite{gardentCreatingTrainingCorpora2017,gardentWebNLGChallengeGenerating2017} contains graph-structured RDF triples from DBPedia \cite{auer2007dbpedia} and their crowdsourced descriptions.  The WebNLG Challenge \cite{gardentCreatingTrainingCorpora2017,ferreira20202020} is a shared task on RDF-to-text generation, which evaluates the systems on the WebNLG dataset.


    \item \textbf{E2E}: The E2E dataset \cite{dusekEvaluatingStateoftheartEndtoEnd2020} contains restaurant descriptions in the form of attribute-value pairs and corresponding human-written recommendations. The name of the dataset is derived from the E2E Challenge, a shared task that focused on evaluating end-to-end NLG systems. \citet{dusekSemanticNoiseMatters2019} show that the original version of the dataset contains semantic noise (incorrect or missing facts in the crowdsourced descriptions) and present a cleaned version of the dataset, which we use for our experiments.


    \item \textbf{Rotowire}: Rotowire \cite{wiseman2017challenges} is a dataset with tabular statistics of basketball games and their corresponding textual summaries, in which only a relevant subset of the input data should be verbalized. Together with the above-average length of the target summaries, this aspect makes the dataset particularly challenging for NLG systems.
\end{itemize}
