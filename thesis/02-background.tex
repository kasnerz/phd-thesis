...
\paragraph{Neural \acp{lm}} A neural \ac{lm} is a \acl{lm} that uses a neural network for estimating the text probability. Denoting the parameters of the network by $\theta$:
\begin{align}
    P(T) = P_\theta(T).
\end{align}
...
The idea behind the \textbf{encoder-decoder framework} \cite{sutskever2014sequence,cho2014learning} is that we can \emph{decode} a sentence using another \ac{rnn} using the hidden state as its initial state.

Abstracting from the \ac{rnn} architecture, the general idea behind the encoder-decoder framework is the following:

\begin{enumerate}
    \item The \textbf{encoder} encodes the input sentence $X$ into a hidden state $h_n$
    \item The \textbf{decoder} uses $h_n$ as its initial state $s_0$ and produces the translation $Y = y_1 \hdots y_m$ of the input sentence by repeatedly applying a transformation $D$ in each timestep $j\in(1,m)$:
          \begin{align}
              s_j, y_j = D(s_{j-1}, y_{j-1}).
          \end{align}
\end{enumerate}

The workflow of an encoder-decoder architecture is depicted in Figure \ref{fig:encdec}. The encoder takes the input sentence and encodes it into a hidden state. In each timestep, the decoder takes the previous hidden state and the last output symbol, and computes the conditional probability distribution over the vocabulary. The symbol with the maximum probability is selected as the output symbol and used as the input for the next step. The first input symbol is a special token \texttt{<s>}  denoting the beginning of the sentence. When the decoder outputs a special token \texttt{<eos>} denoting the end of the sentence, the decoding is finished.

\begin{figure}[ht]
    \centering
    %   \includegraphics[width=0.95\textwidth]{images/encdec}
    \caption[Encoder-Decoder Model]{ \textit{Encoder} encodes the input sentence $X$ into a hidden state, \textit{decoder} uses the hidden state to decode the output sentence $Y$.  Source: \cite{s2s}.}
    \label{fig:encdec}
\end{figure}
