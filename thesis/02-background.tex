...
\paragraph{Sequence-to-Sequence D2T Generation} A general idea in end-to-end \ac{d2t} generation is to use a neural \ac{seq2seq} model for transforming the structured data into text. To get an input sequence suitable for the neural model, the structured data needs to be linearized as a sequence of tokens, potentially with special tokens used as a separator for preserving the original structure of data. An example is presented in \TODO{figure}. The typical architecture for this task is the encoder-decoder framework described in \autoref{sec:transformer}.

\paragraph{RNN-based Approaches}

\paragraph{Transformer-based Approaches}

\paragraph{D2T Generation with Pretrained Models}
