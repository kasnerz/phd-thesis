...
Lastly, we investigate \emph{verbalizing single triples} as a stand-alone task. As we have repeatedly shown (\Cref{sec:iterative,sec:pipeline,sec:sem-acc}), in line with other works \cite{xiangASDOTAnyShotDatatoText2022,kale-rastogi-2020-template,gupta2020infotabs,neeraja2021incorporating}, transforming triples to text helps for \acp{plm}-based data processing. We train a model for verbalizing single relations and use it to replace manual templates in our system described in \autoref{sec:pipeline}.

% The works above use various methods for converting individual triples to text, ranging from simple templates and rule-based systems to prompting \acp{plm}; however, none of them investigate how to apply these approaches to novel relations.
...
\paragraph*{Datasets}  To answer the research questions, we divide our \textsc{Rel2text} dataset into a training and test split. We then use the \textsc{Rel2Text} \emph{test set} to evaluate a finetuned BART model \cite{lewisBARTDenoisingSequencetoSequence2019}. We train the models on the following datasets, all of which focus on verbalizing factual information from \acp{kg} and use the same triple-based input data format:
\begin{itemize}
    ...
    \item KeLM \cite{agarwalKnowledgeGraphBased2021}: A Wikidata-based dataset with 11M examples, 1,519 relations, up to 13 triples per example, and \textit{model-generated} outputs.
          % We use the dataset released by the authors , splitting it into a 1:100 ratio for validation and training data.
\end{itemize}
To answer the question (a), we compare the performance of BART finetuned on the \textsc{Rel2Text} training set with BART finetuned on WebNLG and KeLM. Using \textsc{Rel2text} only, we then prepare various setups for answering (b), (c), and (d).
...
\end{refbox}
In this section, we investigate the specifics of \ac{d2t} generation with \aclp{llm} (\acsp{llm}\glsunset{llm}). To avoid benchmarking the models on data seen during pretraining (\autoref{sec:quintd:data_collection}), we do not use standard \ac{d2t} datasets described in \autoref{sec:datasets}. Instead, we capitalize on the ability of \acp{llm} to process structured data in standard formats such as JSON, CSV, and Markdown without task-specific finetuning. To collect new data, we design a tool for downloading structured data records from public APIs (\Cref{sec:quintd:data_collection,sec:quintd:dataset}).
...
Using reference-free evaluation methods based on human annotators and GPT-4, we evaluate semantic accuracy of model outputs on the word level (\Cref{sec:quintd:experiments,sec:quintd:eval}).
...
Given the available public sources of data, we settled on the five tasks shown in \autoref{tab:quintd:data}: generating a five-day weather forecast, a product description, an ice hockey game summary, a chart caption, and an entity description. The tasks are based on structured data in common formats: JSON, CSV, and Markdown.
...
We instantiate \gptmetric{} with a prompt and a system message describing the task. We instruct the model to produce a JSON output with sequentially ordered errors using the following format: \begin{verbatim}{ "errors": [{ "reason": [REASON], "text": [TEXT_SPAN], 
  "type": [ERROR_CATEGORY] }, ...]}.
\end{verbatim}
% \normalsize
...
A summary of the token-level annotations is given in \Cref{tab:quintd:results_agg,tab:quintd:results_errperex}.
...
\end{itemize}
The correlation on the level of words is weak ($r_{\text{token}}=0.26$) but gets better on the example level ($r_{\text{example}}=0.52$) and even better on the domain level ($r_{\text{domain}}=0.93$).
