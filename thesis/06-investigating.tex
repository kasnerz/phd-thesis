...
\end{refbox}
We present an early analysis of the potential of \ac{d2t} generation with \aclp{llm} (\acsp{llm}\glsunset{llm}). To prevent bias stemming from training data contamination, we depart from standard \ac{d2t} generation benchmarks described in \autoref{sec:datasets}. Instead, we capitalize on the flexibility of \acp{llm} and evaluate the models on generating texts from structured data in standard formats such as JSON, CSV, and Markdown. To collect the data for five \ac{d2t} generation tasks (generating weather forecasts, product descriptions, game summaries, chart captions, and entity descriptions), we design \datatool{} -- a tool for downloading structured data records from public APIs. We build our analysis on referenceless metrics, focusing on evaluating semantic accuracy of model outputs. Furthermore, we put open \acp{llm} in the locus of our investigation, promoting open and reproducible research. We find that although the outputs from the open \acp{llm} are fluent, but the semantic accuracy of the outputs is a major issue: both according to human annotators and our reference-free metric based on GPT-4, more than 80\% of the outputs of open \acp{llm} contain at least one semantic error. We publicly release the code, data, and model outputs.\footnote{\url{https://d2t-llm.github.io/}}
...
We choose the domains so that the data can be directly used as input for five distinct \ac{d2t} generation tasks. Specifically, our tasks include generating weather forecasts, sports reports, product descriptions, chart captions, and entity descriptions (see \autoref{tab:data}).
%
...
For the final experiments, we also included GPT-3.5 (\texttt{gpt-3.5-turbo-1106}) accessed through the OpenAI API \cite{chatgpt}.\footnote{We did not include GPT-3.5 in our preliminary experiments since proprietary models are not our focus. We also did not generate the outputs with GPT-4 because we use the model for evaluation (see §\ref{sec:gpt4eval}) and \acp{llm} tend to be biased towards their outputs \cite{kooBenchmarkingCognitiveBiases2023,stureborgLargeLanguageModels2024}.}
...
\begin{itemize}
    \item \textbf{Focus on semantic accuracy.} The output of \acp{llm} is satisfactory regarding the style, format, and purpose of the text. However, the amount of semantic errors remains very high. Improving the semantic accuracy of the models
          ...
          \item\textbf{Multilinguality is an opportunity.}With the recent efforts in extending \ac{d2t} generation to low-resource languages \cite{cripwell-etal-2023-2023}, multilingual \ac{d2t} generation with open \acp{llm} seems a promising direction. Although we did not go beyond English, initial steps were already done by works such as \citet{lorandi2023data}.
\end{itemize}
